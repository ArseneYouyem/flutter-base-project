...
import '../../../core/services/analitics/analytics_service.dart';
...
Future<bool> handleResponse(Response response,
  ...
  if (response.statusCode == null) {
    ...
    if (response.body["code"] != StatusCode.SUCCESS &&
      ...
      AppSnackbar.showSnackBar("Oups!!!", parseErrorMessageInFrench(message), error: true);
    ...
    if (response.statusCode == 401 &&
      ...
      // }
      if ((showSnackStatus || isPostRequest)) {
        // AppSnackbar.showSnackBar("Acces limité ",
  ...
  try {
    ...
    if ("${response.statusCode}".startsWith("2")) {
      ...
      if (body != null) {
        ...
        if (body["code"] != StatusCode.SUCCESS) AppDialog.closeDialog();
      }
    } else if ("${response.statusCode}".startsWith("5")) {
      if (Get.isSnackbarOpen) Get.closeAllSnackbars();
      if ((showSnackStatus || isPostRequest)) {
